...
def closest_airport(lat,long):
    ...
    """
    latitude_longitude_repo = pd.read_csv("./station_lat_longitude.csv", index_col=0)
...
def weather_odds_helper(kde_pca_accidents,kde_pca_history,weather_now,N=100):
    """
...
def get_odds(lat,long, N= 250):
    ...
    with open('accidents_kde.pickle', 'rb') as f:
        # Load the data from the pickle file
        kde_accidents_dict = pickle.load(f)
    with open('kde_dict.pickle', 'rb') as f:
        # Load the data from the pickle file
    ...
    weather_odds_normal = weather_odds_helper(kde_pca_accidents, kde_pca_history, weather_now,N)
    weather_now_bad = weather_now.copy()
    weather_now_bad["prcp"] = 10
    weather_now_bad["temp"] = 0
    weather_now_bad["pres"] = 980
    print("Densities for weather bad")
    weather_odds_bad = weather_odds_helper(kde_pca_accidents, kde_pca_history, weather_now_bad, N)
